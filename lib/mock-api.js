...
export function normalizeMockProject(input = {}, { id: forcedId } = {}) {
  ...
  const stripLegacyGeneratedIds = Number(input.version) > 0 && Number(input.version) < MOCK_API_VERSION;
...
function applyCollectionRequest(project, { method = "GET", pathname = "/", body, collectionName, singleRecordResponse } = {}) {
  ...
  if (upperMethod === "GET") {
    ...
    const responseBody = filteredRecords.length === 1 && configuredSingleFormat === "object"
      ? filteredRecords[0]
      : filteredRecords.length === 1 && configuredSingleFormat === "array"
        ? filteredRecords
...
function applyEndpointRequest(project, { method = "GET", pathname = "/", body, headers = {} } = {}) {
  ...
  const result = applyCollectionRequest(project, { method: upperMethod, pathname, body, collectionName, singleRecordResponse: endpoint.singleRecordResponse });
