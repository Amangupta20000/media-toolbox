...
export function normalizeMockProject(input = {}, { id: forcedId } = {}) {
  ...
  const collections = sourceCollections.map((collection, collectionIndex) => {
    ...
    const responseShape = collection.responseShape === "object" ? "object" : "array";
    ...
    return { name: collectionName, methods: methods.length ? methods : ["GET"], responseShape, records };
  });
...
function applyCollectionRequest(project, { method = "GET", pathname = "/", body, collectionName } = {}) {
  ...
  if (upperMethod === "GET") {
    ...
    const responseBody = collection.responseShape === "object" ? collection.records[0] || {} : collection.records;
