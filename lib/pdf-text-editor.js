...
function patchTextOperator(operator, operand, encodedHex, font, fallbackKey = "") {
  const encoded = `<${encodedHex}>`;
  if (!fallbackKey && operator.name !== "TJ") return { start: operand.start, end: operand.end, replacement: encoded };
  ...
  if (operator.name === "Tj") return { start: operator.start, end: operator.end, replacement: `${fallbackFont} ${encoded} Tj ${originalFont}` };
  if (operator.name === "TJ") return { start: operator.start, end: operator.end, replacement: `${fallbackKey ? `${fallbackFont} ` : ""}${replacementTextArray(operand, encoded)}${fallbackKey ? ` ${originalFont}` : ""}` };
  if (operator.name === "'") return { start: operator.start, end: operator.end, replacement: `T* ${fallbackFont} ${encoded} Tj ${originalFont}` };
  if (operator.name === "\"") {
    const wordSpacing = operator.operands[0]?.raw ?? "0";
    const characterSpacing = operator.operands[1]?.raw ?? "0";
    return { start: operator.start, end: operator.end, replacement: `${wordSpacing} Tw ${characterSpacing} Tc T* ${fallbackFont} ${encoded} Tj ${originalFont}` };
  }
...
export async function applyPdfTextEdits(input, edits = [], options = {}) {
  ...
  const pageData = document.getPages().map((page, pageIndex) => ({ page, ...collectPageRuns(page, pageIndex, document.context, sourceHash) }));
  ...
  for (const edit of edits) {
    ...
    if (seen.has(run.runId)) throw new Error("A text run was edited more than once in the same export.");
    seen.add(run.runId);
    ...
    if (!run.editable) throw new Error(`“${run.text.slice(0, 60)}” cannot be safely edited: ${run.reason}`);
    ...
    if (encodedHex === null) {
      ...
      const fallbackFont = await document.embedFont(fallbackFontName);
      try {
      ...
      fallbackSequence += 1;
      warnings.push(`Used the bundled ${fallbackFontName} fallback for “${run.text.slice(0, 60)}”. The original font could not encode all replacement characters.`);
    }
    const operator = operatorList(page.content).find((item) => item.start === run.operatorStart && item.end === run.operatorEnd && item.name === run.operator);
    ...
    const patch = patchTextOperator(operator, operand, encodedHex, { ...font, fontSize: run.fontSize }, fallbackKey);
    if (!patchesByPage.has(pageIndex)) patchesByPage.set(pageIndex, []);
    patchesByPage.get(pageIndex).push(patch);
    const originalWidth = Math.max(1, run.text.length);
    if (replacement.length > originalWidth) warnings.push(`Replacement text for “${run.text.slice(0, 60)}” may overflow its original area. Surrounding content was not reflowed.`);
  }
