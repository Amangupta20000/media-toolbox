...
function hasMovement(movement) {
  const x = Number(movement?.pdfX ?? movement?.x);
  const y = Number(movement?.pdfY ?? movement?.y);
  return Number.isFinite(x) && Number.isFinite(y) && (Math.abs(x) > 0.0001 || Math.abs(y) > 0.0001);
}
...
function movedOperator(operator, replacement, movement) {
  ...
  return {
    ...
    replacement: `q 1 0 0 1 ${pdfNumber(movement.pdfX)} ${pdfNumber(movement.pdfY)} cm ${replacement} Q`,
  };
}
...
function displayOffsetToPdf(page, movement) {
  ...
  if (angle === 90) return { pdfX: y, pdfY: x };
  if (angle === 180) return { pdfX: -x, pdfY: y };
  if (angle === 270) return { pdfX: -y, pdfY: -x };
  return { pdfX: x, pdfY: -y };
}
  ...
  for (const edit of edits) {
    ...
    for (const groupRuns of groupedRunsByGroup) {
      for (const [groupIndex, groupedRun] of groupRuns.entries()) {
        ...
        const movement = displayOffsetToPdf(page.page, { x: Number(edit?.offsetX) || 0, y: Number(edit?.offsetY) || 0 });
