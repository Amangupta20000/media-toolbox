...
async function embedBundledFallbackFont(document, family, baseFontOrFormat) {
...
function formattedTextBody(operator, encoded, format, fontKey, alignmentShift = 0) {
  const value = encoded ? (String(encoded).startsWith("<") ? String(encoded) : `<${encoded}>`) : "<>", empty = !encoded;
  const text = empty ? (operator.name === "TJ" ? "[]" : "()") : value;
...
function patchTextOperator(operator, operand, encodedHex, font, fallbackKey = "", movement = null, format = null, formattedFontKey = "", alignmentShift = 0) {
  ...
  if (format && formattedFontKey) {
    const body = formattedTextBody(operator, encodedHex, format, formattedFontKey, alignmentShift);
    return movedOperator(operator, `q ${body} Q`, movement) || { start: operator.start, end: operator.end, replacement: `q ${body} Q` };
  }
...
export async function applyPdfTextEdits(input, edits = [], options = {}) {
  ...
  for (const edit of edits) {
    ...
    const format = hasTextFormat(edit?.format) ? normalizeTextFormat(edit.format, run) : null;
    ...
    const replacement = String(edit?.replacementText ?? run.text ?? "");
    ...
    if (format) {
      const definition = textFormatFontName(format);
      const fontObject = await (definition === "Helvetica" || definition === "Helvetica-Bold" || definition === "Helvetica-Oblique" || definition === "Helvetica-BoldOblique" || definition === "Times-Roman" || definition === "Times-Bold" || definition === "Times-Italic" || definition === "Times-BoldItalic" || definition === "Courier" || definition === "Courier-Bold" || definition === "Courier-Oblique" || definition === "Courier-BoldOblique"
        ? document.embedFont(definition)
        : embedBundledFallbackFont(document, format.fontFamily, format));
      if (!fontObject) throw new Error(`The selected ${format.fontFamily} font could not be embedded in the PDF.`);
      formattedFont = fontObject;
      formattedFontKey = ensureFallbackFontResource(document, page.page, formattedFont, fallbackSequence);
      fallbackSequence += 1;
    }
    const encoded = formattedFont ? formattedFont.encodeText(replacement).toString().replace(/^<|>$/g, "") : encodedTextFor(replacement, font);
    ...
    if (format && formattedFont) {
      ...
      const textWidth = formattedFont.widthOfTextAtSize(replacement, Number(format.fontSize) || 18) + Math.max(0, Array.from(replacement).length - 1) * (Number(format.characterSpacing) || 0);
    ...
    for (const groupRuns of groupedRunsByGroup) {
      for (const [groupIndex, groupedRun] of groupRuns.entries()) {
        ...
        const patch = patchTextOperator(operator, operand, firstInDuplicate ? encodedHex : "", firstInDuplicate ? { ...font, fontSize: run.fontSize } : { key: "", fontSize: 0 }, firstInDuplicate ? fallbackKey : "", firstInDuplicate ? movement : null, firstInDuplicate ? format : null, firstInDuplicate ? formattedFontKey : "", firstInDuplicate ? alignmentShift : 0);
        if (!patchesByPage.has(pageIndex)) patchesByPage.set(pageIndex, []);
        patchesByPage.get(pageIndex).push(patch);
      }
    }
