  ...
  for (const edit of edits) {
    ...
    for (const groupRuns of groupedRunsByGroup) {
      for (const [groupIndex, groupedRun] of groupRuns.entries()) {
        ...
        const patch = patchTextOperator(operator, operand, firstInDuplicate ? encodedHex : "", firstInDuplicate ? { ...font, fontSize: run.fontSize } : { key: "", fontSize: 0 }, firstInDuplicate ? fallbackKey : "");
