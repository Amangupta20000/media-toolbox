...
export async function applyPdfTextEdits(input, edits = [], options = {}) {
  ...
  for (const edit of edits) {
    ...
    const movement = displayOffsetToPdf(page.page, { x: Number(edit?.offsetX) || 0, y: Number(edit?.offsetY) || 0, scale: edit?.scale, rotation: edit?.rotation, originX: edit?.originX, originY: edit?.originY });
    if (edit?.moveOnly) {
