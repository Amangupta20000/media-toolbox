...
export function failInterruptedJobs() {
  const now = Date.now();
  return getDatabase().prepare(`UPDATE jobs
    SET status = 'failed', progress = 100, stage = 'Interrupted',
        message = 'Processing stopped because the worker restarted. Retry the job to continue.',
        error = 'The worker restarted before processing finished.', updated_at = ?
...
export function replaceRetainedJob(previousId, replacementId, result) {
  const db = getDatabase();
  const transaction = db.transaction(() => {
    const previous = db.prepare("SELECT * FROM jobs WHERE id = ?").get(previousId);
    const replacement = db.prepare("SELECT * FROM jobs WHERE id = ?").get(replacementId);
    if (!previous || !replacement || previous.tool !== replacement.tool || previous.status !== "completed") return false;

    let previousOptions;
    let replacementOptions;
    try {
      previousOptions = JSON.parse(previous.options_json || "{}");
      replacementOptions = JSON.parse(replacement.options_json || "{}");
    } catch {
      return false;
    }
    if (previousOptions.retention !== "keep") return false;

    let previousResult = {};
    try { previousResult = JSON.parse(previous.result_json || "{}"); } catch { previousResult = {}; }
    previousResult.path = null;
    previousOptions.retention = "delete";
    previousOptions.supersededBy = replacementId;
    replacementOptions.retention = "keep";
    const now = Date.now();
    db.prepare("UPDATE jobs SET options_json = ?, result_json = ?, updated_at = ? WHERE id = ?")
      .run(JSON.stringify(previousOptions), JSON.stringify(previousResult), now, previousId);
    db.prepare("UPDATE jobs SET options_json = ?, result_json = ?, updated_at = ? WHERE id = ?")
      .run(JSON.stringify(replacementOptions), JSON.stringify(result), now, replacementId);
    return true;
  });
  return transaction();
}
...
export function getJobForPublic(id) {
  ...
  return {
    ...
    tool: row.tool,
    status: row.status,
    historyStatus: options.historyStatus === "saved" ? "saved" : "completed",
