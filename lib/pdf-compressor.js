...
async function rasterizePdfPages(bytes, profile, { onProgress, allowSearchableText = true, compressionOptions = {} } = {}) {
  ...
  try {
    ...
    for (let index = 1; index <= pdf.numPages; index += 1) {
      ...
      if (info.count > 0) imagePages.push(index);
    ...
    } else if (!imageOnlyPages.length && !heavyPages.length && !(profile === "small" && imagePages.length)) {
      return { changed: false, imageOnly: false, pageCount: pdf.numPages, reason: "no image-heavy pages" };
    }
    const pagesToRasterize = imageOnlyPages.length === pdf.numPages
      ? new Set(imageOnlyPages)
      : new Set(profile === "small" ? imagePages : heavyPages);
