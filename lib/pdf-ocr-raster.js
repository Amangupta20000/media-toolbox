...
export function applyRasterTextEdits(canvas, edits, { availableFonts, onFontMatch } = {}) {
  ...
  for (const edit of edits || []) {
    ...
    prepared.push({ edit, original, mask, placement, replacement, style, foreground, preserved, moveOnly, scaleX, scaleY, rotation });
  }
  ...
  for (const { edit, original, placement, replacement, style, foreground, preserved, moveOnly, scaleX, scaleY, rotation } of prepared) {
    if (moveOnly && preserved) {
      ...
      context.restore();
      const renderedWidth = context.measureText(replacement).width * (style.scaleX || 1) * scaleX;
