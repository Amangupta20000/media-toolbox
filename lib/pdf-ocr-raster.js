...
export function applyRasterTextEdits(canvas, edits, { availableFonts, onFontMatch } = {}) {
  ...
  for (const edit of edits || []) {
    ...
    prepared.push({ edit, original, mask, placement, replacement, style, foreground, scale, rotation });
  }
  ...
  for (const { edit, original, placement, replacement, style, foreground, scale, rotation } of prepared) {
    if (replacement) {
      context.save();
