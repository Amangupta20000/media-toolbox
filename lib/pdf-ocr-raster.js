...
export function applyRasterTextEdits(canvas, edits, { availableFonts, onFontMatch } = {}) {
  ...
  for (const edit of edits || []) {
    ...
    if (renderStyle && typeof onFontMatch === "function") onFontMatch({ originalText: String(edit?.originalText || ""), ...renderStyle });
    const placement = {
  ...
  for (const { edit, original, placement, replacement, style, foreground, preserved, moveOnly, scaleX, scaleY, rotation, formatting } of prepared) {
    if (moveOnly && preserved) {
      ...
      const renderedTextWidth = measureSpacedText(context, replacement, characterSpacing) * (style.scaleX || 1);
      const availableWidth = Math.max(1, original.x1 - original.x0);
      const alignedOffset = formatting?.alignment === "center" ? (availableWidth - renderedTextWidth) / 2 : formatting?.alignment === "right" ? availableWidth - renderedTextWidth : 0;
      context.textAlign = "left";
      context.textBaseline = "alphabetic";
      ...
      context.scale(scaleX, scaleY);
      context.translate(-centerX, -centerY);
      context.translate(placement.x0 + alignedOffset, baseline);
      context.scale(style.scaleX || 1, 1);
      drawSpacedText(context, replacement, 0, 0, characterSpacing);
      if (formatting?.underline) {
        const underlineY = Math.max(1, style.fontSize * 0.08);
        context.beginPath();
        context.moveTo(0, underlineY);
        context.lineTo(renderedTextWidth / Math.max(0.25, scaleX), underlineY);
      ...
      const renderedHeight = (style.ascent + style.descent) * scaleY;
