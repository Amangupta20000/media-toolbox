...
export function applyRasterTextEdits(canvas, edits, { availableFonts, onFontMatch } = {}) {
  ...
  for (const edit of edits || []) {
    ...
    prepared.push({ edit, original, mask, placement, replacement, style, foreground });
  }
  ...
  for (const { edit, original, placement, replacement, style, foreground } of prepared) {
    if (replacement) {
      context.save();
      ...
      if (placement.x0 + renderedWidth > width || placement.x0 < 0) warnings.push(`OCR replacement for “${String(edit?.originalText || "text").slice(0, 80)}” reaches the page edge and may be clipped there.`);
      context.restore();
    }
