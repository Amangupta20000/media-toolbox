...
function inferredFontGroup(family) {
  ...
  if (/script|hand|chalk|comic|marker|noteworthy|kalam|brush|cursive|chancery|roundhand/.test(name)) return "hand";
...
function fontCandidateEntries(availableFonts) {
  const candidates = [...FONT_CANDIDATES];
  const seen = new Set(candidates.map(({ family }) => family.toLowerCase()));
  // On the agent, include the host font catalog so a PDF made with a less
  // common installed family can still be matched. Browser preview uses the
  // curated list because browsers do not expose the complete OS catalog.
...
function closestFont(canvas, originalText, original, mask, weight, data, foreground, width, height, availableFonts) {
  ...
  let best = null;
  for (const entry of fontCandidateEntries(availableFonts)) {
...
export function applyRasterTextEdits(canvas, edits, { availableFonts, onFontMatch } = {}) {
  ...
  for (const edit of edits || []) {
    ...
    const style = replacement ? textStyle(canvas, context, source.data, original, mask, String(edit?.originalText || ""), replacement, foreground, width, height, availableFonts, fontCache) : null;
  ...
  for (const { edit, original, replacement, style, foreground } of prepared) {
    if (replacement) {
      ...
      context.save();
      context.translate(original.x0, baseline);
      context.scale(style.scaleX || 1, 1);
      context.fillText(replacement, 0, 0);
      context.restore();
      const renderedWidth = context.measureText(replacement).width * (style.scaleX || 1);
