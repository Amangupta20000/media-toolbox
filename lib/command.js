...
export function runCommand(command, args, options) {
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      ...
      if (cancelTimer) clearInterval(cancelTimer);
      resolve({
        code: timedOut ? 124 : cancelled ? 125 : (code ?? 1),
        stdout,
        stderr: timedOut
          ? `${stderr}\nCommand timed out after ${options.timeoutMs} ms.`.trim()
          : cancelled
            ? `${stderr}\nCommand canceled.`.trim()
            : stderr,
      });
    });
    ...
    if (typeof options?.cancelWhen === "function") {
      cancelTimer = setInterval(() => {
        try {
          if (options.cancelWhen()) stopChild();
        } catch {
          // A cancellation probe must never crash the command runner.
        }
      }, 250);
      cancelTimer.unref?.();
    }
