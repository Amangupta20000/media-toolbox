    ...
    for (const paragraph of block.paragraphs || []) {
      for (const line of paragraph.lines || []) {
        for (const word of line.words || []) {
          ...
          // decorative rules into a short symbol-only "word". Keep those
          // pixels in the page artwork instead of making them editable or
          // merging them into the adjacent sentence.
          if (!text || !word.bbox || isLikelyGraphicWord(word, text)) continue;
          const bbox = {
...
export async function applyPdfOcrEdits(bytes, edits, { sourceHash = "", runSourceHash = sourceHash, password = "" } = {}) {
  ...
  for (const edit of normalized) {
    ...
    if (!Number.isFinite(scale) || scale < 0.25 || scale > 4 || !Number.isFinite(rotation) || Math.abs(rotation) > 100000) throw new Error("OCR text size and rotation must be within the supported range.");
    const list = pagesToEdit.get(pageIndex) || [];
    list.push({ ...edit, pageIndex, bbox: box, offsetX, offsetY, scale, rotation });
