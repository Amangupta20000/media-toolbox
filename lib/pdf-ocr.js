...
export async function recognizePdfText(bytes, { password = "", onProgress } = {}) {
  ...
  const reportProgress = (message) => {
    ...
    const pageShare = 0.9 / Math.max(1, pageCount);
    const pageStart = activePageIndex < 0 ? 0 : 0.1 + activePageIndex * pageShare;
    emitProgress({
  ...
  try {
    for (let index = 0; index < document.numPages; index += 1) {
      ...
      emitProgress({ status: "page", pageIndex: index, progress: Math.round(((index + 1) / document.numPages) * 100), message: `OCR scanned page ${index + 1} of ${document.numPages}.` });
    }
  } finally {
  ...
  return {
    ...
    warnings: ["This PDF has no embedded text layer. OCR edits reconstruct only affected page regions and cannot guarantee the original font, opacity, or hidden image pixels."],
  };
}
...
export async function applyPdfOcrEdits(bytes, edits, { sourceHash = "", password = "" } = {}) {
  const input = bytesFor(bytes);
  ...
  for (const edit of normalized) {
    ...
    const expectedRunId = runMatch ? ocrRunId(pageIndex, Number(runMatch[2]), String(edit.originalText || ""), box, actualHash) : "";
  ...
  try {
    for (let index = 0; index < document.numPages; index += 1) {
      ...
      const rendered = await renderPage(document, index + 1);
      warnings.push(...applyRasterTextEdits(rendered.canvas, editsForPage));
