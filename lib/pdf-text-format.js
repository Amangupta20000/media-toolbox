...
export function fontFamilyFromPdfName(value) {
  ...
  if (/times|serif|roman/.test(name)) return "Times-Roman";
  return "Helvetica";
}

export function textFormatDefaults(run = {}) {
  const baseFont = String(run.baseFont || run.fontFamily || "");
  return {
    fontFamily: FONT_VALUES.has(run.fontFamily) ? run.fontFamily : fontFamilyFromPdfName(baseFont),
    fontSize: Math.max(1, Math.min(500, Number(run.fontSize) || Number(run.height) || 18)),
