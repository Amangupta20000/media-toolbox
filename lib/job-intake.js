...
export function parseMultipart(request, jobDir, { fileSize = Math.max(appConfig.videoMaxBytes, appConfig.pdfMaxBytes, appConfig.pdfImageMaxBytes), maxFiles = 256 } = {}) {
...
async function createPdfEditorJob({ id, jobDir, fields, files, unlimitedPdfEditor = false }) {
  const pdfFiles = files.filter((file) => file.field === "pdf");
  if (!unlimitedPdfEditor && pdfFiles.length > MAX_PDF_COUNT) throw new Error(`You can add up to ${MAX_PDF_COUNT} PDFs at a time.`);
  for (const file of pdfFiles) {
    if (!likelyFileForTool(file, "pdf-editor")) throw new Error(`${file.name} is not a PDF.`);
    if (!unlimitedPdfEditor && file.size > appConfig.pdfMaxBytes) throw new Error(`${file.name} is larger than the 200 MB PDF limit.`);
  }
  const totalPdfBytes = pdfFiles.reduce((total, file) => total + file.size, 0);
  if (!unlimitedPdfEditor && totalPdfBytes > appConfig.pdfTotalMaxBytes) throw new Error("The combined PDF upload is larger than the 200 MB limit.");
...
export async function createJobFromMultipart({ id = randomUUID(), jobDir, fields, files, allowRemoteMediaUrl = false, unlimitedPdfEditor = false }) {
  ...
  if (tool === "pdf-editor") {
    return { ...(await createPdfEditorJob({ id, jobDir, fields, files, unlimitedPdfEditor })), tool };
  }
