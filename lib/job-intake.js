...
export function likelyFileForTool(file, tool) {
  ...
  const allowed = tool === "image-converter" ? imageExtensions : tool === "svg-to-png" ? svgExtensions : tool === "video-repair" ? videoExtensions : pdfExtensions;
  const mime = String(file.mime || "");
  const mimePrefix = tool === "image-converter" ? "image/" : tool === "svg-to-png" ? "image/svg+xml" : tool === "video-repair" ? "video/" : "application/pdf";
...
export async function createJobFromMultipart({ id = randomUUID(), jobDir, fields, files }) {
  ...
  if (!["image-converter", "svg-to-png", "video-repair", "pdf-editor", "pdf-text-editor", "pdf-compressor"].includes(tool)) throw new Error("Choose a supported tool.");
  if (tool === "pdf-editor") {
