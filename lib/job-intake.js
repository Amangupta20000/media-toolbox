...
function validateImagePlacement(image, width, height) {
  ...
  if (![x, y].every((value) => Number.isFinite(value)) || x < -maxPdfImageCoordinate || y < -maxPdfImageCoordinate || x + imageWidth > maxPdfImageCoordinate || y + imageHeight > maxPdfImageCoordinate) {
    throw new Error("The image placement is outside the supported PDF coordinate range.");
  }
...
async function createPdfEditorJob({ id, jobDir, fields, files }) {
  ...
  if (totalPdfBytes > appConfig.pdfTotalMaxBytes) throw new Error("The combined PDF upload is larger than the 50 MB limit.");
...
function parsePdfTextEdits(value) {
  ...
  return edits.map((edit) => {
    ...
    if (mode === "ocr") {
      ...
      if (!String(edit?.originalText || "") || !values.every(Number.isFinite) || values[0] < 0 || values[1] < 0 || values[2] <= values[0] || values[3] <= values[1] || values[2] > 100000 || values[3] > 100000) throw new Error("Each OCR edit must include its original text and a valid page region.");
      return { pageIndex, runId, originalTextHash, originalText: String(edit.originalText), replacementText, mode, bbox: { x0: values[0], y0: values[1], x1: values[2], y1: values[3] }, confidence: Number(edit?.confidence) || 0 };
    }
    return { pageIndex, runId, originalTextHash, replacementText, mode };
  });
}
...
async function createPdfTextEditorJob({ id, fields, files }) {
  ...
  if (source.size > appConfig.pdfMaxBytes) throw new Error(`${source.name} is larger than the 50 MB PDF limit.`);
