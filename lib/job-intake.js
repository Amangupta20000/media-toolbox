...
function validateTextBox(textBox, pageIndex, textBoxIndex) {
  ...
  return { id: String(textBox.id || `text-box-${pageIndex + 1}-${textBoxIndex + 1}`), text, x, y, width, height, fontSize, fontFamily, bold: Boolean(textBox.bold), italic: Boolean(textBox.italic), underline: Boolean(textBox.underline), color: color.toLowerCase(), backgroundColor: backgroundColor === "transparent" ? "transparent" : backgroundColor.toLowerCase(), runs: normalizedRuns };
}
...
async function createPdfEditorJob({ id, jobDir, fields, files }) {
  ...
  createJob({ id, tool: "pdf-editor", sourcePath: manifestPath, sourceName: "PDF editor project", options: { pdfCount: pdfFiles.length, pageCount: pages.length, outputFilename, retention: fields.retention === "keep" ? "keep" : "delete" } });
  return { ids: [id] };
}
...
function parsePdfTextEdits(value) {
  ...
  return edits.map((edit) => {
    ...
    if (mode === "ocr") {
      ...
      return { pageIndex, runId, originalTextHash, originalText, replacementText, mode, offsetX, offsetY, scale, scaleX, scaleY, rotation, bbox: { x0: values[0], y0: values[1], x1: values[2], y1: values[3] }, confidence: Number(edit?.confidence) || 0 };
    }
    return { pageIndex, runId, originalTextHash, originalText, ...(moveOnly ? { moveOnly: true } : { replacementText }), mode, offsetX, offsetY, scale, scaleX, scaleY, rotation, ...(operatorOrdinals ? { operatorOrdinals } : {}), ...(operatorGroups ? { operatorGroups } : {}) };
  });
}
...
const pdfCompressionProfiles = new Set(["balanced", "small", "quality"]);
...
async function createPdfCompressorJob({ id, fields, files }) {
  ...
  createJob({
    ...
    sourcePath: source.path,
    sourceName: source.name,
    options: { compressionProfile, outputFilename, retention: fields.retention === "keep" ? "keep" : "delete" },
  });
  return { ids: [id] };
}
...
export async function createJobFromMultipart({ id = randomUUID(), jobDir, fields, files }) {
  ...
  if (!["image-converter", "video-repair", "pdf-editor", "pdf-text-editor", "pdf-compressor"].includes(tool)) throw new Error("Choose a supported tool.");
  if (tool === "pdf-editor") {
