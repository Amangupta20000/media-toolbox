...
async function createPdfEditorJob({ id, jobDir, fields, files }) {
  ...
  for (const file of pdfFiles) {
    ...
    if (file.size > appConfig.pdfMaxBytes) throw new Error(`${file.name} is larger than the 15 MB PDF limit.`);
  }
...
async function createPdfTextEditorJob({ id, fields, files }) {
  ...
  if (source.size > appConfig.pdfMaxBytes) throw new Error(`${source.name} is larger than the 15 MB PDF limit.`);
