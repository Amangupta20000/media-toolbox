...
function parsePdfTextEdits(value) {
  ...
  return edits.map((edit) => {
    ...
    if (mode === "ocr") {
      ...
      return { pageIndex, runId, originalTextHash, originalText, replacementText, mode, bbox: { x0: values[0], y0: values[1], x1: values[2], y1: values[3] }, confidence: Number(edit?.confidence) || 0 };
    }
    return { pageIndex, runId, originalTextHash, originalText, replacementText, mode, ...(operatorOrdinals ? { operatorOrdinals } : {}), ...(operatorGroups ? { operatorGroups } : {}) };
  });
}
