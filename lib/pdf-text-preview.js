import { PDFArray, PDFDict, PDFDocument, PDFRawStream, PDFName, StandardFonts, decodePDFRawStream } from "pdf-lib";
  ...
  for (const edit of edits) {
    ...
    for (const ordinals of operatorGroups) {
      ...
      if (replacement === null && !format) {
        ...
        if (replacement === null) {
          try {
            const standardFallbackFont = await document.embedFont(fallbackStandardFont(firstFont.baseFont));
            replacement = `<${standardFallbackFont.encodeText(replacementText).toString().replace(/^<|>$/g, "")}>`;
            fallbackKey = ensureFallbackFontResource(document, page.page, standardFallbackFont, pageIndex + 1);
          } catch {
            replacement = null;
          }
        }
        ...
        const body = operator.name === "TJ"
          ...
          : operator.name === "'"
            ...
            : operator.name === "\""
              ...
              : `${fallbackPrefix}${emptyReplacement ? "()" : normalReplacement} Tj${fallbackSuffix}`;
        const usesFallbackFont = Boolean(fallbackKey && index === 0 && !emptyReplacement);
        const patch = moveOperator(operator, body, movement) || (usesFallbackFont
          ? { start: operator.start, end: operator.end, replacement: body }
          : { start: operand.start, end: operand.end, replacement: emptyReplacement ? (operator.name === "TJ" ? "[]" : "()") : normalReplacement });
        if (!patchesByPage.has(pageIndex)) patchesByPage.set(pageIndex, []);
        patchesByPage.get(pageIndex).push(patch);
      }
    }
