...
export async function createPdfTextPreview(input, edits = []) {
  ...
  for (const edit of edits) {
    ...
    const textOperators = operatorList(page.content).filter((item) => TEXT_OPERATORS.has(item.name));
    const replacement = encodeWinAnsi(String(edit?.replacementText ?? ""));
    ...
    for (const ordinals of operatorGroups) {
      ...
      for (const [index, operator] of selectedOperators.entries()) {
        ...
        const encoded = index === 0 ? replacement : encodeWinAnsi("");
