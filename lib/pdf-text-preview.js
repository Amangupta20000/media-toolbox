...
function fontDetails(page, context, resourceName) {
  ...
  if (!font) return { key: resourceName, mappings: new Map(), codeWidth: 1, winAnsi: false };
...
function moveOperator(operator, replacement, movement) {
  ...
  return {
    ...
    replacement: `q 1 0 0 1 ${pdfNumber(movement.x)} ${pdfNumber(movement.y)} cm ${replacement} Q`,
  };
}
...
export async function createPdfTextPreview(input, edits = []) {
  ...
  for (const edit of edits) {
    ...
    for (const item of operatorList(page.content)) {
      ...
      if (TEXT_OPERATORS.has(item.name)) textOperators.push({ ...item, fontKey: activeFontKey });
    }
    ...
    for (const ordinals of operatorGroups) {
      ...
      const replacement = encodedTextFor(String(edit?.replacementText ?? ""), firstFont);
      if (replacement === null) {
        const character = Array.from(String(edit?.replacementText ?? "")).find((value) => !firstFont.mappings?.size || !firstFont.mappings.has(value));
        throw new Error(`The live preview cannot encode “${character || String(edit?.replacementText ?? "") || "the replacement"}” in this PDF font yet. Export still supports the normal fallback-font path.`);
      }
      ...
      for (const [index, operator] of selectedOperators.entries()) {
        ...
        const body = operator.name === "TJ"
          ? `${emptyReplacement ? "[]" : normalReplacement} TJ`
          : operator.name === "'"
            ? `T* ${emptyReplacement ? "()" : normalReplacement} Tj`
            : operator.name === "\""
              ? `${operator.operands[0]?.value ?? "0"} Tw ${operator.operands[1]?.value ?? "0"} Tc T* ${emptyReplacement ? "()" : normalReplacement} Tj`
              : `${emptyReplacement ? "()" : normalReplacement} Tj`;
