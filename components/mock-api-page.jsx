...
export function MockApiPage() {
  ...
  const projectWithDraft = (baseProject = project, draftValue = draft) => {
    ...
    if (databaseMode) {
      ...
      nextCollections = existing ? nextCollections.map((item) => item.name === databaseCollection ? { ...item, methods, responseShape, records } : item) : [...nextCollections, { name: databaseCollection, methods: [draftValue.method], responseShape, records }];
