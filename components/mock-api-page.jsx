...
export function MockApiPage() {
  ...
  return <AppShell><article className="mock-api-page">
    ...
    {surface === "builder" && <PostmanWorkbench project={project} draft={draft} endpoints={endpoints} isDatabase={isDatabase} response={response} savedUrl={savedUrl} exampleHeaders={exampleHeaders} curlText={curlText} onDraftChange={setDraftValue} onNewApi={() => newApi(true)} onSelectEndpoint={selectEndpoint} onRun={runRequest} onSave={saveProject} onImportCurl={importCurl} onCurlChange={setCurlText} onCopy={copy} onExport={exportCurrent} onImport={importFile} onClearDraft={clearDraft} fileInput={fileInput} onProjectChange={(name) => setProject((current) => ({ ...current, name }))} onProjectIdChange={(id) => setProject((current) => ({ ...current, id }))} />}
