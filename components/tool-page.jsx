...
import { AlertTriangle, CheckCircle2, ChevronLeft, ChevronRight, Clock3, Download, FileCheck2, Info, LoaderCircle, LockKeyhole, RotateCcw, ShieldCheck, Sparkles, Zap } from "lucide-react";
...
export function ToolPage({ tool }) {
  ...
  const isVideo = tool === "video-repair";
  ...
  const title = isImage ? "Free private image converter" : isPdfCompressor ? "Free PDF compressor" : "Repair damaged MP4, MOV, and video files locally";
  const eyebrow = isImage ? "Convert without uploads" : isPdfCompressor ? "Reduce PDF size" : "Recover damaged video";
  const description = isImage ? "Convert JPG, JPEG, PNG, GIF, or BMP images in this browser, or use Local agent for HEIC, TIFF, larger files, and full desktop format support." : isPdfCompressor ? "Reduce PDF file size with a balanced browser profile or advanced Local agent profiles while keeping the original PDF untouched." : "Use the Local agent to repair readable video containers, rebuild missing metadata, and create a new file without changing your original video.";
  ...
  const selectProcessingMode = (mode) => {
    ...
    if (mode === "browser" && isPdfCompressor && compressionProfile !== "balanced") setCompressionProfile("balanced");
  ...
  const submit = async () => {
    ...
    if (isPdfCompressor && processingMode === "browser" && compressionProfile !== "balanced") { setCompressionProfile("balanced"); setError("Browser mode supports the Balanced PDF compression profile only."); return; }
  ...
  return <AppShell>
    ...
    {isVideo && <section className="tool-quick-start video-quick-start" aria-labelledby="video-quick-start-title"><div className="tool-quick-start-heading"><span className="section-kicker"><span className="kicker-line" /> Before you start</span><strong id="video-quick-start-title">Video repair runs on the Local agent</strong></div><p className="tool-quick-start-intro">Browser mode is not available for this tool because recovery uses native desktop tools and may need more storage than a browser can safely provide.</p><ol className="tool-quick-start-steps"><li><b>1</b><span><strong>Start the agent</strong><small>Install, open, and authorize it on this computer.</small></span></li><li><b>2</b><span><strong>Add the damaged video</strong><small>Keep the original file until the result is checked.</small></span></li><li><b>3</b><span><strong>Add a reference if needed</strong><small>Use a healthy recording with matching settings.</small></span></li><li><b>4</b><span><strong>Preview the result</strong><small>Check video, audio, duration, and warnings.</small></span></li></ol><div className="tool-quick-start-note"><Info size={16} /><span><strong>Local-agent limit:</strong> videos up to 2 GB. <Link href="/how-to-setup-agent">View the setup guide</Link> before starting if the agent is not connected.</span></div></section>}
    ...
    {!job && !batchJobs && <div className="trust-row">{isVideo ? <><div><CheckCircle2 size={16} /> Original stays untouched</div><div><Clock3 size={16} /> Check the recovered result</div><div><ShieldCheck size={16} /> Local recovery pipeline</div></> : <><div><CheckCircle2 size={16} /> No resizing by default</div><div><Clock3 size={16} /> Temporary processing only</div><div><ShieldCheck size={16} /> Private worker pipeline</div></>}</div>}
...
export function JobStatusCard({ job, tool, isImage, isPdfCompressor, mode, keepResult, onReset }) {
  ...
  return <section className={`job-card ${done ? "success" : failed ? "failed" : ""}`}>
    ...
    {done && job.result && <div className="result-summary"><div><span>Output</span><strong>{job.result.filename}</strong></div><div><span>Size</span><strong>{formatBytes(job.result.bytes)}</strong></div>{isPdfCompressor && Number.isFinite(job.result.reductionPercent) && <div><span>Saved</span><strong>{job.result.reductionPercent > 0 ? `${job.result.reductionPercent}%` : "Already optimized"}</strong></div>}{isPdfCompressor && job.result.targetSizeMb && <div><span>Size target</span><strong>{job.result.targetMet ? `Under ${job.result.targetSizeMb} MB` : "Not reached"}</strong></div>}{isImage && job.result.targetSizeKb && <div><span>Size target</span><strong>{job.result.targetMet ? `Near ${job.result.targetSizeKb} KB` : "Not reached"}</strong></div>}{isImage && job.result.width && <div><span>Resolution</span><strong>{job.result.width} × {job.result.height}</strong></div>}{isImage && job.result.inputFormat === "svg" && job.result.scale && <div><span>Export size</span><strong>{job.result.scale === "custom" ? `${job.result.width} × ${job.result.height} px` : `${job.result.scale}× intrinsic`}</strong></div>}{isImage && job.result.inputFormat === "svg" && job.result.background && <div><span>Background</span><strong>{job.result.background === "color" ? "Solid colour" : job.result.background === "gradient" ? "Gradient" : "Transparent"}</strong></div>}{!isImage && !isPdfCompressor && job.result.durationMs !== null && job.result.durationMs !== undefined && Number.isFinite(Number(job.result.durationMs)) && <div><span>Duration</span><strong>{formatDuration(job.result.durationMs)}</strong></div>}<div><span>Method</span><strong>{job.result.method || "Completed"}</strong></div></div>}
