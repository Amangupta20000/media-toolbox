...
export function LocalAgentSetup() {
  ...
  useEffect(() => {
    // Poll only while the browser still needs pairing. Once connected, the
    // manual Check connection button is the only way to probe the agent.
    if (status === null || status?.connected || pairing) return undefined;
  ...
  const pairingStep = showInstallStep ? "02" : "01";
  const capabilityRows = [
    ...
    ["Untrunc (optional)", capabilities?.video?.untrunc],
  ];
