...
export function LocalAgentSetup() {
  ...
  return <AppShell>
    ...
    <section className="agent-install-card"><div className="card-heading"><div><span className="card-index">01</span><h2>Install the agent</h2></div><span className="optional-label">macOS · Windows · Linux</span></div><p className="card-description">Start here. Download the installer for the computer that should do the processing, install it, and launch the agent. It runs in the background, starts at login, and listens only on this computer.</p><div className="agent-download-grid"><a className="secondary-button" href={releasesUrl} target="_blank" rel="noreferrer"><Download size={17} /> macOS installer</a><a className="secondary-button" href={releasesUrl} target="_blank" rel="noreferrer"><Download size={17} /> Windows installer</a><a className="secondary-button" href={releasesUrl} target="_blank" rel="noreferrer"><Download size={17} /> Linux installer</a></div><div className="agent-macos-note"><AlertTriangle size={17} /><span><strong>macOS first launch:</strong> The public macOS build is not Apple-notarized yet. If macOS says the app is “damaged”, copy it to Applications, then run <code>xattr -dr com.apple.quarantine "/Applications/Media Toolbox Agent.app"</code> in Terminal and open it again.</span></div><div className="agent-security-note"><ShieldCheck size={17} /><span>Pairing grants this website access only to jobs created by this browser. It cannot run shell commands or read arbitrary paths.</span></div></section>
