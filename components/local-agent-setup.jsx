...
export function LocalAgentSetup() {
  ...
  const locked = agentFound && authorization && !authorization.authorized;
  ...
  return <AppShell>
    ...
    {agentFound && !status?.connected && <section className="agent-pair-card"><div className="card-heading"><div><span className="card-index">02</span><h2>Authorize this agent</h2></div><span className={locked ? "required-label" : "optional-label"}>{locked ? "Action required" : "Automatic"}</span></div><p className="card-description">The website requests a separate session automatically after the agent trusts this website origin. For a locked agent, open the desktop dashboard and choose Admin login or enter an activation code. A five-minute trial starts only when you first use processing.</p><div className="agent-open-actions"><a className="secondary-button" href="mediatoolbox://dashboard"><ExternalLink size={16} /> Open agent dashboard</a><span>Credentials and license codes stay inside the desktop application.</span></div></section>}
