...
import { AlertTriangle, BookOpen, CheckCircle2, Download, ExternalLink, Laptop, LoaderCircle, LogOut, RefreshCw, ShieldCheck, XCircle } from "lucide-react";
...
export function LocalAgentSetup() {
  ...
  return <AppShell>
    <div className="page-heading"><div><div className="section-kicker"><span className="kicker-line" /> Local processing</div><h1>Private local file processing</h1><p>Process images, videos, and PDFs on your Mac, Windows, or Linux computer while using this website as the browser interface.</p></div><div className="heading-note"><ShieldCheck size={16} /><span>Files stay on this device</span><Link className="text-link" href="/how-to-setup-agent">Setup guide</Link><Link className="text-link" href="/browser-vs-local-agent">Compare modes</Link></div></div>
    ...
    {showInstallStep && <section className="agent-install-card"><div className="card-heading"><div><span className="card-index">01</span><h2>Install the agent</h2></div><span className="optional-label">macOS, Windows &amp; Linux</span></div><p className="card-description">Download the installer for the computer that should do the processing, install it, and launch the agent. It runs in the background, starts at login, and listens only on this computer. These links always redirect to the matching installer in the latest GitHub release.</p><div className="agent-guide-entry"><div><BookOpen size={18} /><span><strong>New to NativeMedia Agent?</strong><small>Follow the platform-specific setup steps with visual guidance.</small></span></div><Link className="secondary-button" href="/how-to-setup-agent">Open setup guide <ExternalLink size={15} /></Link></div><div className="agent-download-grid"><a className="secondary-button" href={AGENT_DOWNLOAD_PATHS.macos}><Download size={17} /> macOS installer</a><a className="secondary-button" href={AGENT_DOWNLOAD_PATHS.windows}><Download size={17} /> Windows installer</a><a className="secondary-button" href={AGENT_DOWNLOAD_PATHS.linux}><Download size={17} /> Linux installer</a></div><div className="agent-https-note"><ShieldCheck size={17} /><span>Use the latest agent release for Safari and other secure production websites. It creates a trusted HTTPS connection to this device automatically.</span></div>{!macBuildSigned && <DismissibleMessage className="agent-macos-note" resetKey="macos-first-launch-warning"><AlertTriangle size={17} /><div className="agent-macos-note-copy"><strong>macOS first launch</strong><p>If macOS says it cannot verify “NativeMedia Agent.app”:</p><ol><li>Open the app once from Applications.</li><li>Click <strong>Done</strong> in the warning.</li><li>Open <strong>System Settings → Privacy &amp; Security</strong>.</li><li>Click <strong>Open Anyway</strong> for the agent, then confirm <strong>Open</strong>.</li></ol><small>This is normally needed once for a fresh full installation. Future launches and in-app updates use the existing update flow.</small></div></DismissibleMessage>}<div className="agent-security-note"><ShieldCheck size={17} /><span>The website never receives Admin credentials or activation codes. Authorization is managed inside the Local agent dashboard.</span></div></section>}
    ...
    <ToolSeoContent pathname="/local-agent" />
    <ToolFaqContent pathname="/local-agent" />
  </AppShell>;
}
