...
import { CheckCircle2, Download, ExternalLink, Laptop, LoaderCircle, RefreshCw, ShieldCheck, TerminalSquare, XCircle } from "lucide-react";
...
export function LocalAgentSetup() {
  ...
  return <AppShell>
    ...
    {!status?.connected && <section className="agent-pair-card"><div><div className="card-heading"><div><span className="card-index">01</span><h2>Pair this browser</h2></div><span className="optional-label">One time</span></div><p className="card-description">Open the agent from the menu bar or system tray, choose “Show pairing code”, and enter the 6-digit code here.</p></div><form className="agent-pair-form" onSubmit={pair}><label htmlFor="pairing-code">Pairing code</label><div><input id="pairing-code" value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, "").slice(0, 6))} inputMode="numeric" placeholder="123456" autoComplete="one-time-code" /><button className="primary-button" disabled={pairing}>{pairing ? <LoaderCircle className="spin" size={17} /> : <TerminalSquare size={17} />} Pair agent</button></div></form><div className="agent-open-actions"><a className="secondary-button" href="mediatoolbox://pair"><ExternalLink size={16} /> Open agent</a><span>Already installed? This opens the desktop agent.</span></div></section>}
    <section className="agent-capabilities-card"><div className="card-heading"><div><span className="card-index">02</span><h2>Available tools</h2></div><span className={status?.connected ? "optional-label" : "required-label"}>{status?.connected ? "Detected on this device" : "Waiting for agent"}</span></div><div className="agent-capabilities-grid">{capabilityRows.map(([label, available]) => <div className="agent-capability-row" key={label}><span className={`capability-dot ${available ? "ready" : ""}`} /><strong>{label}</strong><span>{available === undefined ? "Not checked" : available ? "Available" : "Unavailable"}</span></div>)}</div><p className="card-description">Optional tools are reported clearly. Image and video jobs are never moved to another machine silently.</p></section>
    <section className="agent-install-card"><div className="card-heading"><div><span className="card-index">03</span><h2>Install the agent</h2></div><span className="optional-label">macOS · Windows · Linux</span></div><p className="card-description">Download the installer for the computer that should do the processing. The agent runs in the background, starts at login, and listens only on this computer.</p><div className="agent-download-grid"><a className="secondary-button" href={releasesUrl} target="_blank" rel="noreferrer"><Download size={17} /> macOS installer</a><a className="secondary-button" href={releasesUrl} target="_blank" rel="noreferrer"><Download size={17} /> Windows installer</a><a className="secondary-button" href={releasesUrl} target="_blank" rel="noreferrer"><Download size={17} /> Linux installer</a></div><div className="agent-security-note"><ShieldCheck size={17} /><span>Pairing grants this website access only to jobs created by this browser. It cannot run shell commands or read arbitrary paths.</span></div></section>
