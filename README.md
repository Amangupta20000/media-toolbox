...
Every signed runtime manifest also declares an `updateType`. For safety, the release workflow defaults to `full`, which tells the dashboard that the complete desktop installer is required. Set the GitHub Actions repository variable `AGENT_UPDATE_TYPE` to `runtime` only for a release whose changes can be replaced entirely from application data. The dashboard blocks runtime installation for full releases and clearly instructs the user to download and install the full release. Developer ID-signed macOS builds continue to use the full Electron updater for these releases.
