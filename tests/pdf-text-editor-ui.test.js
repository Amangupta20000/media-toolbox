...
test("PDF text editor is exposed as a dedicated tool with history navigation", async () => {
  ...
  assert.match(navigation, /aria-expanded=\{!sidebarHidden\}/);
  ...
  assert.match(styles, /\.topbar-brand \{[^}]*min-width: 220px/);
  assert.match(styles, /\.topbar-brand-main \{[^}]*gap: 12px/);
  assert.match(styles, /\.topbar-brand \.brand-mark \{[^}]*width: 38px[^}]*height: 38px/);
