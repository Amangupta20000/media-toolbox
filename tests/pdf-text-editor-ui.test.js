...
test("PDF text editor supports constrained browser replacement and identity-checked local edits", async () => {
  ...
  assert.ok(shell.indexOf('className="pdf-text-retention-row"') < shell.indexOf('className="pdf-text-toolbar"'), "the saved-file row should precede the PDF text tools row");
  ...
  assert.match(await read("styles/globals.css"), /\.pdf-text-page-frame \{[^}]*height: auto[^}]*min-height: 0[^}]*overflow: visible/);
  assert.match(await read("styles/globals.css"), /\.pdf-text-page-frame\[data-preview-zoomed="false"\] \.pdf-text-page-surface \{[^}]*max-width: 100%/);
  ...
  assert.match(shell, /PDF_TEXT_THUMBNAIL_SLOT_EXTRA = 42/);
  assert.match(shell, /function estimatePdfTextThumbnailSlot/);
  assert.match(shell, /typeof itemSize === "function"/);
  assert.match(shell, /windowed\.sizes\[index\]/);
  assert.doesNotMatch(shell, /const itemSize = horizontal \? 145 : 235/);
  assert.match(shell, /useVariablePreviewWindow/);
  assert.match(shell, /new ResizeObserver/);
  assert.match(shell, /getBoundingClientRect\(\)\.height/);
  assert.match(shell, /buildPreviewOffsets/);
  assert.match(shell, /VirtualizedPdfTextPreview/);
  assert.match(shell, /VirtualizedPdfTextRail/);
  assert.match(shell, /function PdfTextThumbnail\(\{ model, onSelect, onKeyDown, elementRef \}\)/);
  assert.match(shell, /const thumbnailRefs = useRef\(new Map\(\)\)/);
  assert.match(shell, /const handleThumbnailKeyDown = \(event, index\) =>/);
  assert.match(shell, /onKeyDown=\{\(event\) => handleThumbnailKeyDown\(event, index\)\}/);
  ...
  assert.match(shell, /useVariablePreviewWindow/);
  assert.match(shell, /windowed\.offsets\[index\]/);
  assert.match(shell, /windowed\.sizes\[index\]/);
  assert.doesNotMatch(shell, /useEstimatedPreviewPageHeight/);
