...
test("PDF text editor is exposed as a dedicated tool with history navigation", async () => {
  ...
  assert.match(navigation, /<img className="brand-logo" src="\/media-toolbox-logo\.png" alt="NativeMedia Agent logo" title="NativeMedia Agent" \/>/);
  ...
  assert.match(navigation, /className="sidebar-menu-button"/);
  ...
  assert.match(navigation, /aria-expanded=\{!sidebarHidden \|\| mobileOpen\}/);
  assert.match(navigation, /aria-expanded=\{!sidebarHidden && mobileOpen\}/);
  ...
  assert.match(styles, /\.topbar-menu-button \{[^}]*display: none/);
  assert.match(styles, /\.topbar-menu-button \{[^}]*position: absolute[^}]*top: 31px[^}]*left: var\(--sidebar-menu-left\)/);
  assert.match(styles, /@media \(min-width: 681px\) \{[\s\S]*\.sidebar-hidden \.topbar-brand \{ margin-left: calc\(var\(--sidebar-menu-left\) \+ 62px\); \}/);
