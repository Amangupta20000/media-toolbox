...
test("public site surfaces have legal links and SEO metadata", async () => {
  ...
  assert.match(setupGuide, /How to install NativeMedia Agent/);
  ...
  assert.match(home, /Free private PDF and media tools/);
  assert.match(home, /href="\/pdf-editor"/);
  assert.match(home, /href="\/image-converter"/);
  assert.match(home, /href: "\/svg-to-png", label: "Free SVG to PNG converter"/);
  assert.match(home, /Rasterize SVG artwork with transparent, solid, or gradient backgrounds/);
  ...
  assert.match(privacy, /Server mode/);
  assert.match(privacy, /adssettings\.google\.com/);
  assert.match(privacy, /aboutads\.info/);
  ...
  assert.match(terms, /Processing modes/);
  ...
  assert.match(contact, /personal or confidential information/);
...
test("SEO metadata is route-specific and normalizes query strings", () => {
  ...
  assert.equal(metadataForPathname("/pdf-editor?tab=history").title, "Free PDF Editor — Merge & Organize Pages | NativeMedia Agent");
