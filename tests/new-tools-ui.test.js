...
test("new local tools are routed, indexable, and excluded from Browser mode", async () => {
  ...
  assert.match(page, /Use a media URL/);
  ...
  assert.match(seo, /Convert video to audio files or public media URLs/);
  assert.match(page, /I own this content or have permission to download and convert it/);
  assert.match(page, /Server mode or the Local agent/);
