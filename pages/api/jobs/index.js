...
import { config as appConfig, paths, supportedImageFormats } from "../../../lib/config.js";
...
function likelyFileForTool(file, tool) {
  ...
  return allowed.has(extension(file.name)) || file.mime.startsWith(mimePrefix);
}

function parseMultipart(request, jobDir) {
  return new Promise((resolve, reject) => {
    const contentType = request.headers["content-type"];
    if (!contentType?.includes("multipart/form-data")) return reject(new Error("Expected a multipart upload."));
    const parser = Busboy({ headers: { "content-type": contentType }, limits: { files: 256, fields: 12, fileSize: Math.max(appConfig.videoMaxBytes, appConfig.pdfMaxBytes, appConfig.pdfImageMaxBytes) } });
    const fields = {};
    const files = [];
    const pending = [];
    let fileOrder = 0;
    ...
    parser.on("finish", async () => {
      try { await Promise.all(pending); files.sort((left, right) => left.order - right.order); resolve({ fields, files }); } catch (error) { reject(error); }
    });
    request.pipe(parser);
  });
}
...
function parseImageAttachments(operation, imageByField, width, height, pageIndex) {
  ...
  return requestedImages.map((imageOperation, imageIndex) => {
    ...
    if (!imageFile || (!imageFile.mime.startsWith("image/") && !pdfImageExtensions.has(extension(imageFile.name)))) throw new Error(`Image ${imageIndex + 1} on page ${pageIndex + 1} is not a supported image.`);
...
async function createPdfEditorJob({ id, jobDir, fields, files }) {
  ...
  const pages = operations.map((operation, index) => {
    ...
    if (operation.kind === "source") {
      ...
      if (!Number.isInteger(pdfIndex) || pdfIndex < 0 || pdfIndex >= pdfFiles.length || !Number.isInteger(pageIndex) || pageIndex < 0) {
        throw new Error(`Page ${index + 1} points to an invalid PDF page.`);
      }
      const page = { kind: "source", pdfIndex, pageIndex };
      const requestedImages = Array.isArray(operation.images) || operation.imageField;
      if (requestedImages) {
    ...
    const page = { kind: "blank", width, height, rotation, images: parseImageAttachments(operation, imageByField, width, height, index) };
    return page;
  });
  ...
  createJob({ id, tool: "pdf-editor", sourcePath: manifestPath, sourceName: "PDF editor project", options: { pdfCount: pdfFiles.length, pageCount: pages.length } });
}
...
export default async function handler(request, response) {
  ...
  try {
    ...
    if (tool !== "image-converter" && tool !== "video-repair" && tool !== "pdf-editor") throw new Error("Choose a supported tool.");
    ...
    if (tool === "image-converter") {
      ...
      createJob({ id, tool, sourcePath: source.path, sourceName: source.name, options: { format, method, maxSizeKb, jpegConfirmed: fields.jpegConfirmed === "true" } });
    } else {
      ...
      createJob({ id, tool, sourcePath: source.path, sourceName: source.name, referencePath: reference?.path, referenceName: reference?.name, options: { hasReference: Boolean(reference) } });
    }
    return response.status(202).json({ jobId: id, status: "queued" });
  } catch (error) {
